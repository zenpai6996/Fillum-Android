...
export function TrendingTv({ data}) {
    ...
    return (
        ...
        <View className="mb-8 ">
            <Carousel
                ...
                autoPlayInterval={3000}
                data={data}
                renderItem={({ item , animationValue}) => <TvCard item={item} handleClick={handleClick} animationValue={animationValue}/>}
                height={height*0.7}
                width={width}
                mode="parallax"
                style={{ alignItems: 'center' ,justifyContent:'center', alignSelf:'center'}}
...
const TvCard = ({ item , handleClick , animationValue}) => {
    const animatedStyle = useAnimatedStyle(() =>{
        const opacity = interpolate(
            animationValue.value,
            [-1,0,1],
